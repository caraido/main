...
from analysis.helpers._cross_patient_helpers import (
...
CONDITION_COLORS = {"transfer": "#1f77b4", "no_transfer": "#d62728", "pca_align": "#ff7f0e", "original": "#2ca02c"}
CONDITION_LABELS = {
    "transfer": "Condition 1: transfer",
    "no_transfer": "Condition 2: kernel PLS (no transfer)",
    "pca_align": "Condition 3: PCA align",
    "original": "Condition 4: X full data (existing baseline)",
}
METRICS_OVERALL = [
...
def plot_time_courses(df_pair, baseline_df, target, embedding,
    ...
    for ax, (col, title) in zip(axes, METRICS):
        for condition in ["transfer", "no_transfer", "pca_align"]:
            condition_sub = sub[sub["arm"] == condition]
            if condition_sub.empty:
                continue
            agg = condition_sub.groupby("time_bin")[col].agg(["mean", "std", "count"])
            t_s = agg.index.values * bin_size_ms / 1000.0
            ax.plot(t_s, agg["mean"], lw=1.8, color=CONDITION_COLORS[condition], label=CONDITION_LABELS[condition])
            sem = agg["std"] / np.sqrt(agg["count"].clip(lower=1))
            ax.fill_between(t_s, agg["mean"] - sem, agg["mean"] + sem,
                            color=CONDITION_COLORS[condition], alpha=0.18)
        if baseline_df is not None:
            col_map = {
                ...
                # unseen-mode column names (Condition 3 is a full-data ceiling — same
                # underlying CSV columns; the per_time_scores file doesn't
                # distinguish seen/unseen because Condition 3 sees every word)
                "cosine_unseen":     "cosine_mean",
                "word_acc_unseen":   "word_balanced_acc",
                "cat_acc_unseen":    "category_balanced_acc",
            }
            base_col = col_map.get(col)
            if base_col and base_col in baseline_df.columns:
                t_s3 = baseline_df["bin_index"].values * bin_size_ms / 1000.0
                ax.plot(t_s3, baseline_df[base_col].values, lw=1.5, ls="--",
                        color=CONDITION_COLORS["original"], label=CONDITION_LABELS["original"])
...
def plot_sample_efficiency(df_pair, target, embedding, chance: dict | None = None,
    ...
    for ax, (col, title) in zip(axes, METRICS):
        for condition in ["transfer", "no_transfer", "pca_align"]:
            condition_sub = peak[peak["arm"] == condition]
            if condition_sub.empty:
                continue
            agg = condition_sub.groupby("k")[col].agg(["mean", "std", "count"])
            ax.plot(agg.index, agg["mean"], "-o", color=CONDITION_COLORS[condition],
                    label=CONDITION_LABELS[condition], lw=1.8, markersize=5)
            sem = agg["std"] / np.sqrt(agg["count"].clip(lower=1))
        ...
        # Condition 3 horizontal ceiling line (peak value)
        if baseline_df is not None:
            col_map = {
                "cosine_mean":       "cosine_mean",
                "word_bal_acc":      "word_balanced_acc",
            ...
            base_col = col_map.get(col)
            if base_col and base_col in baseline_df.columns:
                ceil_peak = float(np.nanmax(baseline_df[base_col].values))
                ax.axhline(ceil_peak, ls="--", lw=1.4,
                           color=CONDITION_COLORS["original"],
                           label=f"{CONDITION_LABELS['original']} ({ceil_peak:.3f})")
...
def plot_seen_vs_unseen(df_pair, target, embedding) -> str:
    ...
    for ax, (c_s, c_u, title) in zip(axes, metric_cols):
        for condition, marker in (("transfer", "o"), ("no_transfer", "x"), ("pca_align", "^")):
            sub = peak[peak.arm == condition]
            ...
            ax.errorbar(agg["k"], agg["seen"], yerr=agg["seen_std"], marker=marker, ls="-",
                        color=CONDITION_COLORS[condition], label=f"{CONDITION_LABELS[condition]} (seen)", alpha=0.9, capsize=2)
            ax.errorbar(agg["k"], agg["unseen"], yerr=agg["unseen_std"], marker=marker, ls=":",
                        color=CONDITION_COLORS[condition], label=f"{CONDITION_LABELS[condition]} (unseen)", alpha=0.5, capsize=2)
        ax.set_xlabel("k")
        ax.set_title(title, fontsize=10)
...
def _records_for(records, condition, k):
    return [r for r in records if r["arm"] == condition and r["k"] == k]
...
def plot_svd_spectrum(records, target, embedding) -> str:
    ...
    for j, k in enumerate(ks):
        ...
        for r in sub:
            s = r["svd"]["s"]
            ax.plot(np.arange(1, len(s) + 1), s, color=CONDITION_COLORS["transfer"], alpha=0.25, lw=0.8)
        ...
        ax2.hist(eff, bins=20, color=CONDITION_COLORS["transfer"], alpha=0.7)
    ...
    fig.suptitle(f"{target} / {embedding} - SVD of M_X (Condition 1 transfer)", fontsize=11)
    fig.tight_layout()
...
def plot_quiver_anchors(payload, target, embedding, k_for_quiver=None) -> str:
    ...
    # If pred_anchors are in source HGA space (CCA condition), project them to T space
    # using the stored PLS encoder and x_mean before applying the shared PCA.
    pls_encoder = meta.get("pls_encoder")   # (n_feat_src, n_comp) or None
    pls_x_mean  = meta.get("pls_x_mean")   # (n_feat_src,) or None
    def _to_t_space(arr: np.ndarray):
        """Project arr to T space if it is in HGA space (CCA case).
        Returns None if the array is in HGA space but no encoder is available
        (old pkl files), so the caller can skip the record."""
        if arr.shape[-1] != pca.n_features_in_:
            if pls_encoder is not None:
                return (arr - pls_x_mean) @ pls_encoder
            return None  # old pkl — cannot project; skip this record
        return arr
    word_pred_2d = {}
    # pred_anchors are per-word predicted values; average by word first
    for r in sub:
        ...
        for w, ps in word_preds_r.items():
            projected = _to_t_space(np.mean(ps, axis=0, keepdims=True))
            if projected is None:
                continue
            p2 = pca.transform(projected)[0]
...
def plot_rotation_consistency(records, target, embedding) -> str:
    ...
    for k in ks:
        sub = _records_for(transfer, "transfer", k)
        u_vecs = [r["svd"]["U"][:, 0] for r in sub]
        min_len = min(v.shape[0] for v in u_vecs)
        U1 = np.stack([v[:min_len] for v in u_vecs], axis=0)
    ...
    ax.set_title(f"{target} / {embedding} - rotation consistency (Condition 1)", fontsize=10)
    ...
    for tgt in targets:
        ...
        for emb in embeddings:
            ...
            if baseline_run is not None:
                baseline_df = load_arm3_baseline(tgt, emb, baseline_run,
                                              results_root=arm3_results_root)
                if baseline_df is None:
                    step(f"  Condition 3 missing for {tgt}/{emb}/{baseline_run}.")
                if show_chance:
                    chance = load_arm3_chance(tgt, emb, baseline_run,
                                              results_root=arm3_results_root)
            parts.append(f"<h3>{tgt} / {emb}</h3>")
            parts.append("<h4>1. Time courses</h4>")
            parts.append(plot_time_courses(pair, baseline_df, tgt, emb,
                                            chance=chance))
            ...
            if len(stats) > 0:
                ...
                parts.append("<details><summary>Wilcoxon Condition1 &gt; Condition2 stats</summary>")
...
def main():
    ...
    parser.add_argument("--baseline-run", default=None,
                        help="Run folder name under --arm3-results-root for the "
                             "Condition 3 (full-data per-patient PLS) overlay.")
    parser.add_argument("--arm3-results-root", default=DEFAULT_ARM3_RESULTS_ROOT,
                        help="Directory containing Condition 3 per-run subfolders. "
    ...
    parser.add_argument("--show-chance", dest="show_chance", action="store_true",
                        ...
                        help="Plot shuffled-cosine + 1/N theoretical chance lines "
                             "from the Condition 3 run dir (default ON; requires "
                             "--baseline-run).")
    ...
    import _archive.cross_patient_decoding.cross_patient_few_shot_report as _self
