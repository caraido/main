...
| [`cross_task_reports/`](cross_task_reports/) (2 scripts) | 2026-07 | HTML-report generators superseded by `figures_for_paper/cross_task/cross_task_panels.py`, which produces the publication figures from the same CSVs. The *compute* scripts they wrapped were promoted to `analysis/cross_task/`. **`cross_task_channel_importance_report.py`'s inputs are also archived now** (see next row). | `cross_task_channel_importance_results/` |
| `cross_task_channel_importance_results/` (result CSVs/PNGs + 2 HTML reports) | 2026-07-20 | The per-**channel** cross-task importance analysis was retired: under the Nystroem-RBF dilution single-channel effects are weak (almost every channel lands in `neither`), so the analysis is now purely ROI/region-based (`analysis/cross_task/cross_task_region_importance.py`, renamed from `cross_task_channel_importance.py`). Moved out of the live `results/cross_task_cotrain/` tree: `channel_importance_all.csv`, `channel_pls_vip_all.csv`, `channel_importance_group_counts.csv`, both `channel_importance_report*.html`, and per-patient `channel_*` CSVs/PNGs (6 patients, 45 files). Gitignored (`*results`), so this row is the tracked provenance record. | `cross_task_channel_importance_results/` |
| [`legacy/seen_unseen_analysis.py`](legacy/seen_unseen_analysis.py) | 2026-07-28 | Exploratory diagnostic that split ordinary random-test trials by whether their word happened to occur in that split's training set. It produced no saved result in the current pinned picture run and was superseded by the controlled word-level holdout and open-gallery evaluation in `analysis/open_vocab_retrieval/`. | No standalone result; authoritative source data are under `figures/open_vocab_retrieval/source_data/`. |
