...
def objective_full(trial):
    svc_c = trial.suggest_float("svc_c", 1e-6, 1e10, log=True)
    svc_gamma = trial.suggest_float("svc_gamma", 1e-10, 1e6, log=True)
    extractor_obj = extractor_name(n_components=trial.suggest_int("n_components", 1, 40))
    classifier_obj = SVC(kernel='rbf', C=svc_c, gamma=svc_gamma, class_weight='balanced')
    accuracy = 0.0
    for i in range(len(X_to_use)):
        X_train_to_tune, _, y_train_to_tune, _ = train_test_split(X_to_use[i], labels_to_use, test_size=0.001)
        X_train_to_tune = StandardScaler().fit_transform(X_train_to_tune)
        X_train_to_tune = extractor_obj.fit_transform(X_train_to_tune, y_train_to_tune)
        score = cross_val_score(classifier_obj, X_train_to_tune, y_train_to_tune, n_jobs=-1, cv=5)
        accuracy = np.maximum(score.mean(), accuracy)
    return accuracy
...
if __name__ == "__main__":
    ...
    data_folder = 'data'
    patient = 'MBF'
    what_to_decode = 'Morphology'# this could be "SubjectNum", "SubjectPerson" "Morphology", "Tense"
    ...
    # step 1: find the best parameters
    if what_to_decode == 'SubjectNum':
        labels_to_use = sentence_subject_number_label
    elif what_to_decode == 'SubjectPerson':
        labels_to_use=sentence_subject_person_label
    elif what_to_decode == 'Morphology':
        labels_to_use = clean_morpho_label
    elif what_to_decode == 'Tense':
        labels_to_use = sentence_tense_label
    else:
        raise ValueError("what_to_decode should be 'SubjectNum', 'Morphology', or 'Tense'")
    data_to_use = np.array(data_binned_phrase_onset).swapaxes(1, 2)[(labels_to_use!='NA')&(labels_to_use!='first person singular')]
    labels_to_use = labels_to_use[(labels_to_use != 'NA') & (labels_to_use != 'first person singular')]
    ...
    study.optimize(objective_full, n_trials=200,show_progress_bar=True) # be careful of this since the parameters are used from the variable space.
    decoder = SVC(kernel='rbf', class_weight='balanced', C=study.best_params['svc_c'],gamma=study.best_params['svc_gamma'])
    extractor = extractor_name(n_components=study.best_params['n_components'])
    print(study.best_params)

    fig = optuna.visualization.plot_optimization_history(study)
    write_html(fig, f'figures/{what_to_decode}/full_optuna_history_{patient}_phrase_onset_{extractor}.html')
    fig = optuna.visualization.plot_parallel_coordinate(study, params=["svc_gamma", "svc_c"])
    write_html(fig, f'figures/{what_to_decode}/full_optuna_parallel_coordinate_{patient}_phrase_onset_{extractor}.html')
    fig = optuna.visualization.plot_param_importances(study)
    write_html(fig, f'figures/{what_to_decode}/full_optuna_param_importances_{patient}_phrase_onset_{extractor}.html')
    fig = optuna.visualization.plot_contour(study, params=["svc_gamma", "svc_c"])
    write_html(fig, f'figures/{what_to_decode}/full_optuna_contour_{patient}_phrase_onset_{extractor}.html')
    ...
    save_name = f'accuracy_aligned_to_{patient}_phrase_onset_{extractor}_{decoder}_{bin_size}ms.pdf'
    save_name = safe_filename(save_name)
    fig.savefig(f"figures/{what_to_decode}/" + save_name)
    ...
    data_to_use = np.array(data_binned_voice_onset).swapaxes(1, 2)[(labels_to_use!='NA')&(labels_to_use!='first person singular')]
    labels_to_use = labels_to_use[(labels_to_use != 'NA') & (labels_to_use != 'first person singular')]
    n_bins = data_to_use.shape[1]
    ...
    study.optimize(objective_full, n_trials=200,show_progress_bar=True) # be careful of this since the parameters are used from the variable space.
    decoder = SVC(kernel='rbf', class_weight='balanced', C=study.best_params['svc_c'],gamma=study.best_params['svc_gamma'])
    extractor = PCA(n_components=study.best_params['n_components'])
    print(study.best_params)

    fig = optuna.visualization.plot_optimization_history(study)
    write_html(fig, f'figures/{what_to_decode}/full_optuna_history_{patient}_voice_onset_{extractor}.html')
    fig = optuna.visualization.plot_parallel_coordinate(study, params=["svc_gamma", "svc_c"])
    write_html(fig, f'figures/{what_to_decode}/full_optuna_parallel_coordinate_{patient}_voice_onset_{extractor}.html')
    fig = optuna.visualization.plot_param_importances(study)
    write_html(fig, f'figures/{what_to_decode}/full_optuna_param_importances_{patient}_voice_onset_{extractor}.html')
    fig = optuna.visualization.plot_contour(study, params=["svc_gamma", "svc_c"])
    write_html(fig, f'figures/{what_to_decode}/full_optuna_contour_{patient}_voice_onset_{extractor}.html')
    ...
    fig,ax=plot_accuarcy(pipeline.mean_test_accuracy, pipeline.mean_chance,
                  ...
                  data_std=[pipeline.std_test_accuracy, 0],
                  p=p_illustration, n_xticks=10,
                  title=f"aligned to Voice onset")

    save_name=f'accuracy_aligned_to_{patient}_voice_onset_{extractor}_{decoder}_{bin_size}ms.pdf'
    save_name=safe_filename(save_name)
    fig.savefig( f"figures/{what_to_decode}/"+save_name)
